...
def test_parse_section(rfcxml):
    ...
    xml_t = etree.fromstring(t)
    txt = rfcxml.parse_section(xml_t)
...
def test_parse_section_unnumbered(rfcxml):
...
def test_parse_figure(rfcxml):
    ...
    xml_t = etree.fromstring(t)
    txt = rfcxml.parse_figure(xml_t)

    out = "".join(txt)
    assert "~~~\n\nPOST" in out


def test_parse_rfc(rfcxml):
    t = b"""<rfc
    ipr = "trust200902"
...
def test_parse_front(rfcxml):
    ...
    txt = rfcxml.parse_front(front)
    out = "".join(txt)
    assert "workgroup" in out


def test_parse_note(rfcxml):
    note = harn_get_root("note.xml")
    txt = rfcxml.parse_note(note)
    out = "".join(txt)
